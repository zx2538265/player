...
try {
  const stored = JSON.parse(localStorage.getItem('ateez-practice') || '{}');
...
function savePreferences() {
  try { localStorage.setItem('ateez-practice', JSON.stringify({...preferences, favorites:[...favorites]})); } catch {}
}
...
function jumpToCue(index, play = false) {
  ...
  // Do not replay a preceding chant during this cue's lead-in.
  seek(Math.max(bounds().start, cues[index].start - 3, cues[index - 1]?.end || 0));
...
function renderCatalog() {
  ...
  if (!count) { const empty = document.createElement('p'); empty.textContent = songs.length ? '沒有符合的歌曲，試試其他關鍵字或篩選。' : '航線尚未展開，曲目即將加入。'; $('songList').append(empty); }
}
function bounds() {
...
function selectSong(index, updateHash = true, autoplay = null) {
  ...
  $('songTitle').textContent = song.title; document.title = `${song.title} · ATEEZ 應援練習室`;
...
fetch('songs.json?v=20260924-chant-review').then(response => {if(!response.ok) throw new Error('catalog'); return response.json();}).then(data => {
  if (!Array.isArray(data)) throw new Error('catalog');
  songs = data;
  if (!songs.length) {
    $('songMeta').textContent = 'ATINY · 航行準備中';
    $('songTitle').textContent = '下一段冒險，從這裡開始';
    $('status').textContent = '歌單準備中，加入曲目後即可開始練習';
    $('videoContainer').classList.add('empty-voyage');
    $('videoContainer').replaceChildren();
    const mark = document.createElement('span'); mark.className = 'empty-mark'; mark.textContent = '✦';
    const title = document.createElement('strong'); title.textContent = 'READY TO SET SAIL';
    const note = document.createElement('p'); note.textContent = '等待第一首歌，升起我們的旗幟';
    $('videoContainer').append(mark, title, note);
    $('transport').hidden = true;
    document.querySelector('.chant-panel').hidden = true;
    document.querySelector('.continuous-control').hidden = true;
    document.querySelector('.source').hidden = true;
    $('songSelect').append(Object.assign(document.createElement('option'), {textContent:'歌單準備中'}));
