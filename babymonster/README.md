...
- WE GO UP 使用指定影片 `x4b_9YdhT8M`，播放範圍 00:31～03:36，已加入 50 張應援提示；依星號文字與綠色提示逐段核對，快速短句合併顯示，未加入一般歌詞或自製中文空耳
...
- BATTER UP（第 3 首）使用指定影片 `CRSVJA-dKWo`（艾倫Allen），已加入 34 張紅字應援提示；僅取紅字，保留韓文原文，白色歌詞不納入。開場先在心中數兩個八拍
- BATTER UP 已核對 8 張逐秒總覽及 204 張 0.1 秒邊界畫面；提示依字幕出現時間，行內接唱文字可能早於實際喊唱，尚未完成全段聽校與 YouTube 實播驗證。證據與產生程式位於 `video/CRSVJA-dKWo/`；第 26 首 Encore Remix 保持待補
