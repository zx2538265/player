...
- 沿用 `EojU8B2DEL0`，新增 32 張黃色應援提示卡；排除白字一般歌詞與片頭標題
- 字母與密集 Jump／Bump／Yeah 合併顯示；韓文沿用原片空耳，`bulgeum` 使用原片空耳「噗棍」
- 以全片每秒下方畫面、候選段落每 0.5 秒全畫面及 57 個邊界的 0.1 秒取樣人工核對，時間依動畫暫定；抽幀不代表全片逐幀或逐句聽校
- 原片、SHA-256、抽幀與逐段資料保存在 `video/EojU8B2DEL0/chant-ledger.json`；逐句聽校 0/32，YouTube 實播同步與 2026 現場編排尚未驗收

- RINGA LINGA 修訂：補上 63.4–64.0 秒 `left`、64.2–64.9 秒 `right`；以 0.1 秒全畫面核對進出，100.8–102.0 秒改回原片空耳「噗棍」
